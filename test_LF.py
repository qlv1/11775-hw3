...
if __name__ == '__main__':
    ...
    X_score = np.zeros((len(X_test), 3))

    for j, feature in enumerate(features):
        X = X_test[:, dimension_i[j] : dimension_i[j] + dimension_i[j+1]]
    ...
    print (clf.predict(X_score))

    np.savetxt(output_file, clf.decision_function(X_score))
