...
for event in P001 P002 P003; do
  ...
  python train_DF_submit.py $event "DF_vector_submit/" $feat_dim_DF DF_pred_submit/svm.$event.model || exit 1;
  # apply the svm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred
  python test_DF_submit.py DF_pred_submit/svm.$event.model "DF_vector_submit/" $feat_dim_DF DF_pred_submit/${event}_test_DF.lst || exit 1;
