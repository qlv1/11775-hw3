...
for event in P001 P002 P003; do
  ...
  python train_EF_sang.py $event "EF_vector/" $feat_dim_EF EF_pred/svm.$event.model || exit 1;
  # apply the svm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred
  python test_svm_EF_sang.py EF_pred/svm.$event.model "EF_vector/" $feat_dim_EF EF_pred/${event}_val_EF.lst || exit 1;
