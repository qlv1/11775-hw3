...
for event in P001 P002 P003; do
  ...
  python train_svm.py $event "surf_vector/" $feat_dim_surf surf_pred/svm.$event.model || exit 1;
  # apply the svm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred
  python test_svm_val.py surf_pred/svm.$event.model "surf_vector/" $feat_dim_surf surf_pred/${event}_val_surf.lst || exit 1;
...
for event in P001 P002 P003; do
  echo "=========  Event $event  ========="
  # now train a svm model
  python train_svm.py $event "cnn_vector/" $feat_dim_cnn cnn_pred/svm.$event.model || exit 1;
  # apply the svm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred
  python test_svm_val.py cnn_pred/svm.$event.model "cnn_vector/" $feat_dim_cnn cnn_pred/${event}_val_cnn.lst || exit 1;
  # python test_svm_submit.py cnn_pred/svm.$event.model "cnn_vector/" $feat_dim_cnn cnn_pred/${event}_test_cnn.lst || exit 1;
  # compute the average precision by calling the mAP package
  ap ${event}_val_label cnn_pred/${event}_val_cnn.lst
done
