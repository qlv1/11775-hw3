...
for event in P001 P002 P003; do
  ...
  python train_EF_submit.py $event "EF_vector/" $feat_dim_EF EF_pred/svm.$event.test.model || exit 1;
  # apply the svm model to *ALL* the testing videos;
  # output the score of each testing video to a file ${event}_pred
  python test_svm_EF_submit.py EF_pred/svm.$event.test.model "EF_vector/" $feat_dim_EF EF_pred/${event}_test_EF_submit.lst || exit 1;
